...
class CRedisCache extends CCache
{
	...
	public function getRedis()
    {
        if($this->_cache!==null)
            ...
            $conn = $this->_cache->connect($this->servers[0]['host'], $this->servers[0]['port']);
            //	return $this->_cache=new Redis($this->servers);
            if(!empty($conn)&&!empty($this->servers[0]["password"])) {
                $ret = $this->_cache->auth($this->servers[0]["password"]);
                if(empty($ret)) {
                    Yii::log('redis can not auth','warning','app.cache');
                }
                return $ret ? $conn : $ret;
            } else {
                Yii::log('redis can not connect','warning','app.cache');
            }
            return $conn;
        }
    }
